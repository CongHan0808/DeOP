...
class HungarianMatcher(nn.Module):
    ...
    def memory_efficient_forward(self, outputs, targets):
        ...
        for b in range(bs):
            ...
            is_nan_mask = np.isnan(cost_mask.cpu()).any()
            if is_nan_mask:
                raise ValueError("matrix contains nan in mask")
            # Compute the dice loss betwen masks
            cost_dice = batch_dice_loss(out_mask, tgt_mask)
            is_nan_dice = np.isnan(cost_dice.cpu()).any()
            if is_nan_dice:
                raise ValueError("matrix contains nan in dice")

            # is_nan_class = np.isnan(cost_class.cpu()).any()
            # if is_nan_class:
            #     raise ValueError("matrix contains nan in class")
